...
constexpr int gridSize = 21;  // Konstanta untuk ukuran grid
constexpr int maxExpressionSize = 1000;  // Konstanta untuk ukuran buffer ekspresi
...
private:
    ...
    double caclRPN(double x)
    {
        ...
        while (token != NULL)
        {
            ...
            {
                RPNstack.push(atof(token)); // Menggunakan atof daripada std::stod
            }
...
int main()
{
    // Membaca jumlah fungsi yang akan diplot
    int functions;
    scanf("%d", &functions);
